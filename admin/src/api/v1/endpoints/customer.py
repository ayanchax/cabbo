...
from customer_api.src.models.customer.customer_schema import CustomerRead, CustomerSuspensionRequest
from customer_api.src.models.user.user_orm import User
from sqlalchemy.ext.asyncio import AsyncSession

from customer_api.src.services.customer_service import (
...
async def list_customers(
    ...
    if current_user_role not in [RoleEnum.super_admin, RoleEnum.customer_admin]:
        raise CabboException(
            "You do not have permission to view customers.", status_code=403, error_code=UNAUTHORIZED
        )
    ...
    if not customer_model or len(customer_model) == 0:
        raise CabboException(
            status_code=404, message="No customers found with the given filters", error_code=GENERIC_EXCEPTION
        )
...
async def get_customer_profile(
    ...
    if not customer_model:
        raise CabboException(status_code=404, message="Customer not found", error_code=GENERIC_EXCEPTION)
...
async def activate_customer(
    ...
    if current_user_role not in [RoleEnum.super_admin, RoleEnum.customer_admin]:
        raise CabboException(
            "You do not have permission to activate customers.", status_code=403, error_code=UNAUTHORIZED
        )
    success, error_message = await async_activate_customer(
        customer_id=customer_id, db=db
    )
    if error_message:
        raise CabboException(status_code=500, message=error_message, error_code=GENERIC_EXCEPTION)

    if not success:
        raise CabboException(status_code=500, message="Failed to activate customer", error_code=GENERIC_EXCEPTION)
...
async def deactivate_customer(
    ...
    if current_user_role not in [RoleEnum.super_admin, RoleEnum.customer_admin]:
        raise CabboException(
            "You do not have permission to deactivate customers.", status_code=403, error_code=UNAUTHORIZED
        )
    ...
    if error_message:
        raise CabboException(status_code=500, message=error_message, error_code=GENERIC_EXCEPTION)
    if not success:
        raise CabboException(status_code=500, message="Failed to deactivate customer", error_code=GENERIC_EXCEPTION)
...
async def suspend_customer(
    ...
    if current_user_role not in [RoleEnum.super_admin, RoleEnum.customer_admin]:
        raise CabboException(
            "You do not have permission to suspend customers.", status_code=403, error_code=UNAUTHORIZED
        )
    ...
    if error_message:
        raise CabboException(status_code=500, message=error_message, error_code=GENERIC_EXCEPTION)
    if not success:
        raise CabboException(status_code=500, message="Failed to suspend customer", error_code=GENERIC_EXCEPTION)
