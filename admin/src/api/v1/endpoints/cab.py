from fastapi import APIRouter
from fastapi.params import Depends
from sqlalchemy.ext.asyncio import AsyncSession
from customer_api.src.core.exceptions import GENERIC_EXCEPTION, UNAUTHORIZED, CabboException
from customer_api.src.core.security import RoleEnum, validate_user_token
from customer_api.src.db.database import a_yield_mysql_session
from customer_api.src.models.cab.cab_schema import CabTypeSchema, CabTypeUpdateSchema
from customer_api.src.models.user.user_orm import User
from customer_api.src.services.cab_service import add_new_cab_type, async_activate_cab, async_delete_cab_type, async_get_all_cabs, async_update_cab_type, get_cab_type_by_id


router = APIRouter()

# ====== Cab Configuration Endpoints ================

# Add cab type 
@router.post("/add", response_model=CabTypeSchema)
async def add_cab_type(cab_type: CabTypeSchema, db: AsyncSession = Depends(a_yield_mysql_session),current_user: User = Depends(validate_user_token),):
    """Add a new cab type to the system configuration."""
    current_user_role = current_user.role
    
    if current_user_role not in [RoleEnum.super_admin, RoleEnum.driver_admin]:
        raise CabboException(
            "You do not have permission to add cab types.", status_code=403, error_code=UNAUTHORIZED
        )
    
    result = await add_new_cab_type(cab_type=cab_type, db=db, created_by=current_user_role)
    if not result:
        raise CabboException(status_code=500, message="Failed to add new cab type", error_code=GENERIC_EXCEPTION)
    return result

# List cab types
@router.get("/list", response_model=list[CabTypeSchema])
async def list_cab_types(db:AsyncSession = Depends(a_yield_mysql_session), current_user: User = Depends(validate_user_token)):
    """List all cab types in the system configuration."""
    current_user_role = current_user.role
    if current_user_role not in [RoleEnum.super_admin, RoleEnum.driver_admin]:
        raise CabboException(
            "You do not have permission to view cab types.", status_code=403, error_code=UNAUTHORIZED
        )
    return await async_get_all_cabs(db=db)

#Get cab type by id
@router.get("/{cab_type_id}", response_model=CabTypeSchema)
async def get_cab_type(cab_type_id: str, db: AsyncSession = Depends(a_yield_mysql_session), current_user: User = Depends(validate_user_token)):
    """Retrieve a cab type by its ID."""
    current_user_role = current_user.role
    if current_user_role not in [RoleEnum.super_admin, RoleEnum.driver_admin]:
        raise CabboException(
            "You do not have permission to view cab types.", status_code=403, error_code=UNAUTHORIZED
        )
    cab_type = await get_cab_type_by_id(cab_type_id=cab_type_id, db=db)
    if not cab_type:
        raise CabboException(status_code=404, message="Cab type not found", error_code=GENERIC_EXCEPTION)
    return cab_type

# Update cab type
@router.put("/{cab_type_id}", response_model=CabTypeSchema)
async def update_cab_type(cab_type_id: str, payload: CabTypeUpdateSchema, db: AsyncSession = Depends(a_yield_mysql_session), current_user: User = Depends(validate_user_token)):
    """Update an existing cab type's configuration."""
    current_user_role = current_user.role
    if current_user_role not in [RoleEnum.super_admin, RoleEnum.driver_admin]:
        raise CabboException(
            "You do not have permission to update cab types.", status_code=403, error_code=UNAUTHORIZED
        )
    payload.id = cab_type_id
    result = await async_update_cab_type(cab_type_data=payload, db=db)
    if not result:
        raise CabboException(status_code=500, message="Failed to update cab type", error_code=GENERIC_EXCEPTION)
    return result

@router.patch("/{cab_type_id}/activate")
async def activate_cab_type(cab_type_id: str, db: AsyncSession = Depends(a_yield_mysql_session), current_user: User = Depends(validate_user_token)):
    """Activate a cab type in the system configuration."""
    current_user_role = current_user.role
    if current_user_role not in [RoleEnum.super_admin, RoleEnum.driver_admin]:
        raise CabboException(
            "You do not have permission to activate cab types.", status_code=403, error_code=UNAUTHORIZED
        )
    success, error_message = await async_activate_cab(cab_type_id=cab_type_id, db=db)
    if not success:
        raise CabboException(status_code=400, message=error_message or "Failed to activate cab type", error_code=GENERIC_EXCEPTION)
    return {"detail": f"Cab type {cab_type_id} activated successfully."}



# Delete cab type
@router.delete("/{cab_type_id}")
async def delete_cab_type(cab_type_id: str, db: AsyncSession = Depends(a_yield_mysql_session), current_user: User = Depends(validate_user_token)):
    """Delete a cab type from the system configuration."""
    current_user_role = current_user.role
    if current_user_role not in [RoleEnum.super_admin, RoleEnum.driver_admin]:
        raise CabboException(
            "You do not have permission to delete cab types.", status_code=403, error_code=UNAUTHORIZED
        )
    success, error_message = await async_delete_cab_type(cab_type_id=cab_type_id, db=db)
    if not success:
        raise CabboException(status_code=400, message=error_message or "Failed to delete cab type", error_code=GENERIC_EXCEPTION)
    
    return {"detail": f"Cab type {cab_type_id} deleted successfully."}
    

