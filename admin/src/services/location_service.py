...
from customer_api.src.models.map.location_schema import LocationInfo, LocationProximity
...
def get_distance_km(
    ...
    if provider == "google":
        from customer_api.src.services.google_map_service import get_distance_km as google_get_distance
...
def get_location_suggestions(query: str, allowed_countries:List[str], proximity:Union[LocationProximity,None]=None, limit:int=5, session_token:Optional[str]=None) -> List[LocationInfo]:
    ...
    """
    if provider == "google":
        from customer_api.src.services.google_map_service import (
