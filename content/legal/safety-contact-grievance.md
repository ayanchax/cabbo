---
slug: safety-contact-grievance
title: Safety, Contact & Grievance
version: "1.0.0"
effective_date: "2026-07-02"
content_format: markdown
requires_acceptance: false
last_updated: "2026-07-02T00:00:00+05:30"
status: published
display_order: 60
category: safety
locale: en-IN
---

# Safety, Contact & Grievance

Cabbo aims to provide practical support before, during, and after rides. This page explains how to contact us, what to do in safety situations, and how grievances are handled.

## Emergency Disclaimer

Cabbo is not an emergency response provider.

If you face an immediate safety risk, medical emergency, crime, accident, fire, or threat, contact local emergency services, police, ambulance, or other appropriate authorities first.

After you are safe, use the support option on the booking detail page so Cabbo can review and assist with ride-related action.

## Safety Expectations

Customers, drivers, and Cabbo staff are expected to behave respectfully and lawfully.

Customers should:

- Use the booked ride for lawful travel only.
- Avoid abusive, threatening, discriminatory, or unsafe behavior.
- Wear seat belts where required.
- Avoid distracting the driver.
- Not ask drivers to break traffic laws.
- Keep personal belongings secure.
- Use the booking detail support option if driver or vehicle details do not match the booking.

## Driver And Cab Verification

Before starting the ride, check that the driver name, phone number, vehicle number, and vehicle details match the booking details shared by Cabbo where available.

If the details do not match, do not start the ride until you have confirmed through the booking detail support option.

## Contact Cabbo

For trip-specific help, open the booking detail page and use the Call or WhatsApp support option shown there.

For faster help, include:

- Booking ID
- Registered phone number
- Pickup and drop location
- Ride date and time
- Short description of the issue

## Escalation

If your issue is not resolved through regular support, you may ask for escalation. Cabbo may review booking records, support history, payment records, trip status, driver inputs, and other available information.

Escalated issues may take longer where they involve payment partners, drivers, safety review, legal review, or third-party records.

Cabbo will route trip support through the booking detail page support flow. Privacy and grievance requests can be emailed to grievances@cabbo.co.in.

## Grievance Contact

For grievances, email Cabbo at grievances@cabbo.co.in and include:

- Your name
- Registered phone number or email
- Booking ID, if relevant
- Clear description of the concern
- Supporting screenshots, payment references, or documents

Cabbo will acknowledge and review grievances in line with applicable law and internal support processes.

## Privacy And Data Grievances

For privacy-related requests or concerns, such as access, correction, deletion, consent withdrawal, or data-use questions, email Cabbo at grievances@cabbo.co.in.

Some requests may require verification of your identity before Cabbo can act on them.

## False Or Abusive Reports

Cabbo may restrict accounts or take appropriate action where a user repeatedly submits false complaints, abusive messages, threats, fraudulent refund claims, or misuse of support channels.

## Updates To This Page

Cabbo may update safety, contact, escalation, and grievance details as the service grows or legal requirements change.
