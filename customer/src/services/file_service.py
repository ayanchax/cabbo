...
from customer_api.src.services.s3.s3_key_builder import S3KeyBuilder
from customer_api.src.services.s3.s3_service import S3Service
...
def save_file(path: Union[Path, str], content: str = ""):
    ...
    try:
        ...
        log.error(f"Failed to save file at {path}: {str(e)}")
        return False
    return True
...
def copy_file(
    ...
    try:
        ...
        log.error(f"Failed to copy file from {src} to {dest}: {str(e)}")
        return False
