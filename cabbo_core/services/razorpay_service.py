...
from cabbo_core.models.policies.refund_enum import RefundType

from enum import Enum
import logging

import razorpay.errors
from cabbo_core.constants import APP_NAME, APP_VERSION
from cabbo_core.exceptions import RAZORPAY_PAYMENT_ORDER_CREATION_FAILED, CabboException
from cabbo_core.models.customer.customer_orm import Customer
from cabbo_core.models.customer.customer_schema import CustomerPayment
from cabbo_core.models.financial.payments_schema import (
    PaymentNotesSchema,
...
def initialize_client(key_id, key_secret):
    global RAZOR_PAY_CLIENT
    RAZOR_PAY_CLIENT = razorpay.Client(auth=(key_id, key_secret))
    RAZOR_PAY_CLIENT.set_app_details(RAZOR_PAY_CLIENT_DETAILS)
    return RAZOR_PAY_CLIENT


def get_razorpay_client():
    if RAZOR_PAY_CLIENT is None:
...
def _get_razorpay_existing_order(
    ...
    """
    try:
        existing_order = get_razorpay_client().order.fetch(order_id or razorpay_order.receipt)
