...
from services.driver_rating_service import get_average_rating_by_driver_id, list_reviews_by_driver_id
from sqlalchemy.ext.asyncio import AsyncSession

router = APIRouter()

#List unique non-flagged reviews of a driver given by various customers.
@router.get("/driver/{driver_id}/ratings", response_model=list[DriverRatingResponseSchema])
async def list_unique_reviews_of_driver(
    driver_id: str,
    flag:FlagsEnum=Query(FlagsEnum.none, description="Filter reviews based on their flagged status. Use 'flagged' to retrieve only flagged reviews, 'unflagged' to retrieve only non-flagged reviews."),
